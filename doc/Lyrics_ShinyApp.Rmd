...
lyrics.3 <- lyrics.2 %>%
  ...
  summarize(n = sum(sentiment))

### divide by decade  
lyrics.decade <- lyrics.3 %>%
...
lyrics.decade <- lyrics.decade %>% filter(n >-100)
# distribution of magnitute
...
g1 + facet_wrap(~genre)
# by decade
g1 + facet_wrap(~decade)


# frequency
lyrics.freq <- lyrics.2 %>% 
  group_by(word) %>%
  summarize(count = sum(sentiment)) %>%
  arrange(desc(count))
